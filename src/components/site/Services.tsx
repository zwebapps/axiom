      ...
      <PageWrap>
        ...
        <RevealStagger className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3" stagger={0.09}>
          {servicesContent.items.map((s, i) => {
            ...
            return (
              <RevealItem key={s.title} variant="scale" className="h-full">
                <TiltCard>
                  <article className="panel group relative h-full overflow-hidden rounded-xs p-7 transition-colors duration-500 ease-[var(--ease-lux)] hover:border-gold/50">
        ...
        <Reveal delay={120} className="mt-12 flex justify-center" variant="fade">
          <MotionLink href="#contact" className="btn-gold group">
            {servicesContent.cta}
            <ArrowRight
              size={15}
